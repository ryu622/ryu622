...
大学では、グラフニューラルネットワークを用いてサッカーデータを分析する研究しています。（使用言語はPythonでフレームワークはPytorchを使っています。）→gnn-counterattack-xai-v2
...
- gnn-counterattack-xai-v2
  現在製作途中ですが、研究で製作しているものです。GNN（グラフニューラルネットワーク）を用いてサッカーのカウンターアタックの成功予測のタスクを解き、特に機械学習の解釈性にアプローチする研究（XAIというもの）を行っています。具体的には従来の深層学習モデルで課題となっている解釈性やデータサイズの制約にアプローチするために、モデルにヒューリスティックな制約や物理的な制約を課し、アテンション機構におけるアテンション係数の可視化やGNNExplainerという手法を用いて予測に不可欠であったサブグラフや特徴重要度を可視化することによって考察を行います。
- FukuLoop2.0
